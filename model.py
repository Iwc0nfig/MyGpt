...
class MLP(nn.Module):
    def __init__(self, config):
        ...
        # A common practice is to make the hidden_dim a multiple of 256 and use a SwiGLU variant
        
        hidden_dim = int(2 * hidden_dim / 3)
        hidden_dim = 256 * ((hidden_dim + 256 - 1) // 256)

        self.c_fc1 = nn.Linear(config.n_embd, hidden_dim, bias=config.bias)
        self.c_fc2 = nn.Linear(config.n_embd, hidden_dim, bias=config.bias)
        self.c_proj = nn.Linear(hidden_dim, config.n_embd, bias=config.bias)
        self.dropout = nn.Dropout(config.dropout)

    def forward(self, x):
        # SwiGLU activation has shown better result than gelu TODO learn how it works
